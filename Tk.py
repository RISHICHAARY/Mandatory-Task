...
curs=con.cursor()
curs.execute("create table if not exists Accounts(user_name varchar(25),password varchar(25))")
con.commit()
...
def Aut():
        ...
        def SIGN():
                ...
                window1.geometry("550x345")
                window1.overrideredirect(1)
                center_window(550,345)
                ...
                User=Label(Sub,font=("vardana",25,'bold'),text="USERNAME:",bg='#ffffce').pack(padx=30,anchor="nw")
                UsN=Entry(Sub,font=("vardana",25),border=0,width="25")
                UsN.pack(padx=35,pady=10,anchor="ne")
                Pass=Label(Sub,font=("vardana",25,'bold'),text="PASSWORD:",bg='#ffffce').pack(padx=30,anchor="nw")
                PaN=Entry(Sub,font=("vardana",25),border=0,width="25",show="*")
                PaN.pack(padx=35,pady=10,anchor='ne')
                SB=Button(Sub,font=("vardana",15,'bold'),text="CONFIRM",bg="#ffde59",bd=0,command=lambda:[fun(UsN,PaN)]).pack(pady=10)
                def fun(UsN,PaN):
                        a=UsN.get()
                        b1=PaN.get()
                        aa=str(a)
                        ...
                        for i in range(0,rng):
                                if(len(d)!=0):
                                        if(d[i][0]==a):
                                                ...
                                                curs.execute("insert into Accounts values('{}','{}')".format(a,b1))
                                                con.commit()
        ...
        SB_Window=Cvas1.create_window(1030,600,anchor="nw",window=SB)
        LB=Button(window0,font=("vardana",15,'bold'),text="LOGIN",bg="green2",padx=30,bd=0,command=lambda:[LOG(UsN,PaN)])
        LB_Window=Cvas1.create_window(1070,550,anchor="nw",window=LB)
...
def Dashboard(Usr):
        ...
        curs1=con1.cursor()
        curs1.execute("create table if not exists Presdetl(Adno varchar(30),Name varchar(30),Dob varchar(25),Gndr varchar(10),email varchar(30),adrs varchar(100),Fname varchar(30),Fpn varchar(15),Focc varchar(25),Fanualinc varchar(15),Mname varchar(30),Mpn varchar(15),Mocc varchar(25),Manualinc varchar(15))")
        ...
        def prof(A,B,C):
                ...
                Main.pack(side=RIGHT)
                Qt=Button(Main,text="<",bg="#d03636",padx=10,pady=5,command=lambda:[Main.destroy(),Btenb(),Dbr(A,B,C)])
                Qt.pack(pady=5,padx=5,anchor="ne")
        ...
        def atd(A,B,C):
                ...
                Qt=Button(NmH1,text="<",bg="#d03636",padx=10,pady=5,command=lambda:[Main.destroy(),Btenb(),Dbr(A,B,C)])
                Qt.pack(anchor="ne")
